...
do
	for model_type in Trf_ctc_att #Cfm_ctc_att Cfm_hubert Cfm_transducer
	do
		...
		python summarize.py \
			${output_dir}/espnet_rtf.txt # > ${output_dir}/${model_type}/espnet_rtf.txt
		python summarize.py \
			${output_dir}/onnx_rtf.txt #> ${output_dir}/${model_type}/onnx_rtf.txt
	done
done
